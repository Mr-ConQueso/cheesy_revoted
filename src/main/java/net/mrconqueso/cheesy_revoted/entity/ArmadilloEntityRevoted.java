...
public class ArmadilloEntityRevoted extends AnimalEntity implements GeoEntity {
    public ArmadilloEntityRevoted(EntityType<? extends AnimalEntity> entityType, World world) {
    ...
    public static final RawAnimation ROLLUP = RawAnimation.begin().thenLoop("misc.rollup");
    ...
    public <T extends Entity & GeoAnimatable> AnimationController<T> armadilloAnimationController(T entity) {
        return new AnimationController<T>(entity, "Run/Walk/Idle", 1, state -> {
            ...
            if (this.isSprinting()) {
                return state.setAndContinue(ROLLUP);
            }
            if (state.isMoving()) {
