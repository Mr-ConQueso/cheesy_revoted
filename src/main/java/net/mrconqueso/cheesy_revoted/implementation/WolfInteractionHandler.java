package net.mrconqueso.cheesy_revoted.implementation;
        ...
        if(entity instanceof WolfEntity wolf) {
            if(wolf.getOwnerUuid() != null) {
                if(wolf.getOwnerUuid().equals(player.getUuid())) {
                    WolfDataAccessor data = CheesyRevoted.getData(wolf);
                    ...
                    if(data.getWolfArmor().isEmpty() && player.getMainHandStack().getItem() instanceof WolfArmorItem) {
                        if(!world.isClient) {
                            ...
                            wolf.playSound(ModSounds.WOLF_ARMOR_EQUIP, 0.5F, 1.0F);
